    ...
    try:
        if ide_type == IDEType.VSCODE:
            ...
            if system == "Windows":
                ...
                # Add profile directory for additional cleaning
                profile_dir = Path.home() / ".vscode-insiders"
            elif system == "Darwin":  # macOS
            ...
            paths["state_db"] = base_dir / "globalStorage" / "state.vscdb"
            paths["storage_json"] = base_dir / "globalStorage" / "storage.json"
            paths["history"] = base_dir / "History"  # Add history folder for cleaning
            paths["profile_dir"] = profile_dir
            paths["profile_extensions"] = profile_dir / "extensions"
            paths["profile_extensions_json"] = profile_dir / "extensions.json"
